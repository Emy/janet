...
// https://github.com/skyra-project/skyra/blob/master/src/providers/postgres.ts
...
export default class extends SQLProvider {
    ...
    public async createTable(table: string, rows?: readonly [string, string][]) {
    ...
    public async addColumn(table: string, column: SchemaFolder | SchemaEntry) {
    ...
    public async removeColumn(table: string, columns: string | readonly string[]) {
        const escapedTable = this.cIdentifier(table);
    ...
    public async runAll<R>(...sql: readonly unknown[]) {
    ...
    public async runOne<R>(...sql: readonly unknown[]) {
        // eslint-disable-next-line @typescript-eslint/ban-ts-ignore
        // @ts-ignore
        const results = await this.run<R>(...sql);
