...
import { QueryBuilder, SQLProvider, util } from 'klasa';
import { Pool } from 'pg';
...
module.exports = class extends SQLProvider {
    ...
    countRows(table) {
        return this.runOne(`SELECT COUNT(*) FROM ${sanitizeKeyName(table)};`).then((result) => Number(result.count));
    }
    ...
    getAll(table, entries = []) {
        ...
        return this.runAll(`SELECT * FROM ${sanitizeKeyName(table)};`).then((results) =>
            results.map((output) => this.parseEntry(table, output)),
        );
    }

    getKeys(table) {
        return this.runAll(`SELECT id FROM ${sanitizeKeyName(table)};`).then((rows) => rows.map((row) => row.id));
    }
    ...
    get(table, key, value) {
        ...
        return this.runOne(`SELECT * FROM ${sanitizeKeyName(table)} WHERE ${sanitizeKeyName(key)} = $1 LIMIT 1;`, [
            value,
        ]).then((output) => this.parseEntry(table, output));
    }

    has(table, id) {
        return this.runOne(`SELECT id FROM ${sanitizeKeyName(table)} WHERE id = $1 LIMIT 1;`, [id]).then((result) =>
            Boolean(result),
        );
    }
    ...
    getSorted(table, key, order = 'DESC', limitMin, limitMax) {
        return this.runAll(
            `SELECT * FROM ${sanitizeKeyName(table)} ORDER BY ${sanitizeKeyName(key)} ${order} ${parseRange(
                limitMin,
                limitMax,
            )};`,
        );
    ...
    decrementValue(table, id, key, amount = 1) {
        return this.run(`UPDATE ${sanitizeKeyName(table)} SET $2 = GREATEST(0, $2 - $3) WHERE id = $1;`, [
            id,
            key,
            amount,
        ]);
    ...
    removeColumn(table, columns) {
        if (typeof columns === 'string')
            return this.run(`ALTER TABLE ${sanitizeKeyName(table)} DROP COLUMN ${sanitizeKeyName(columns)};`);
        if (Array.isArray(columns))
            return this.run(
                `ALTER TABLE ${sanitizeKeyName(table)} DROP COLUMN ${columns.map(sanitizeKeyName).join(', ')};`,
            );
    ...
    updateColumn(table, piece) {
        ...
        return this.run(
            `ALTER TABLE ${sanitizeKeyName(table)} ALTER COLUMN ${column} TYPE ${datatype}${
                piece.default
    ...
    run(...sql) {
        return this.db.query(...sql).then((result) => result);
    }
